...
- Closed CodeQL alert #3 (`java/log-injection`, `StormtrooperX.java`). The `checkForUpdates` callback logged `currentVersion` / `latestVersion` directly. Both are already shape-validated upstream — `latestVersion` via `UpdateChecker.validateReleaseTag`, `currentVersion` from `plugin.yml` — but the sanitizer sits across the `UpdateChecker` callback boundary, so CodeQL could not trace it to the sink and kept the line flagged. A new `sanitizeForLog` helper strips CR/LF at the logging sink itself: cheap defense-in-depth that also makes the guard local to the sink. (The earlier wording crediting the `validateReleaseTag` change with closing alert #3 was inaccurate — that change sanitized the source side, not the `StormtrooperX` sink.)
